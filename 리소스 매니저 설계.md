...
- 추가적으로 D3D리소스를 멤버로 소유한 리소스들은 엔진 로직에서 개념적으로 쪼갤 수 없는 단계까지 추상화시켜 중복으로 자원이 생성되는 일이 없도록 제한시켰습니다.
...
- Mesh : 정점 및 인덱스 정의
- StaticMesh : Mesh의 모음
- RigidMesh : Mesh와 RigidBone의 모음
- SkinningMesh : Mesh와 SkinningBone의 모음
- RigidBone : 전체 메쉬를 이동하기 위한 본
- SkinningBone : 묶여있는 정점을 이동하기 위한 본
- Textrue : 텍스쳐
- Material : 재질
- AnimationClip : 본 애니메이션 클립
- AssimpModel : FBX, OBJ, ... 등의 컨버팅 전 모델 파일의 캐쉬
